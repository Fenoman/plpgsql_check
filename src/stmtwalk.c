				  ...
				  List *params)
{
	...
	/*
	 * Skip dynamic SQL checking if configured. The execute string is not
	 * parsed here, so we can neither tell whether it is constant, nor
	 * determine the volatility of the query it carries.
	 */
	if (plpgsql_check_disable_dynamic_sql_check)
	{
		...
		/* The query was not analysed, so volatility cannot be determined */
